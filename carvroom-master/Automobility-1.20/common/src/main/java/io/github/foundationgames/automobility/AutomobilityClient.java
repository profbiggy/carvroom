...
import io.github.foundationgames.automobility.entity.*;
import io.github.foundationgames.automobility.entity.render.*;
...
public class AutomobilityClient {
    ...
    public static void initItems() {
        ...
        Platform.get().builtinItemRenderer(AutomobilityItems.BANANA_THROWABLE.require(), (stack, type, pose, buffers, light, overlay) -> {
                ...
                pose.mulPose(Axis.YP.rotation((float) Math.PI));
        ...
        Platform.get().builtinItemRenderer(AutomobilityItems.SHELL_THROWABLE.require(), (stack, type, pose, buffers, light, overlay) -> {

            ShellEntity entity = new ShellEntity(Minecraft.getInstance().level);
            Minecraft.getInstance().getEntityRenderDispatcher().render(entity, 0, 0, 0, 0, 0, pose, buffers, light);


        });

        Platform.get().builtinItemRenderer(AutomobilityItems.MUSHROOM_THROWABLE.require(), (stack, type, pose, buffers, light, overlay) -> {

            MushroomEntity entity = new MushroomEntity(Minecraft.getInstance().level);
            Minecraft.getInstance().getEntityRenderDispatcher().render(entity, 0, 0, 0, 0, 0, pose, buffers, light);
        });

        Platform.get().builtinItemRenderer(AutomobilityItems.BOMB_THROWABLE.require(), (stack, type, pose, buffers, light, overlay) -> {

            BombEntity entity = new BombEntity(Minecraft.getInstance().level);
            Minecraft.getInstance().getEntityRenderDispatcher().render(entity, 0, 0, 0, 0, 0, pose, buffers, light);
        });

        Platform.get().builtinItemRenderer(AutomobilityItems.LIGHTNING_THROWABLE.require(), (stack, type, pose, buffers, light, overlay) -> {

            LightningEntity entity = new LightningEntity(Minecraft.getInstance().level);
            Minecraft.getInstance().getEntityRenderDispatcher().render(entity, 0, 0, 0, 0, 0, pose, buffers, light);
        });
        ...
        Platform.get().builtinItemRenderer(AutomobilityItems.AUTOMOBILE2.require(), (stack, type, pose, buffers, light, overlay) -> {
            automobileReader.read(stack.getOrCreateTagElement("Automobile"));
            float wheelDist = automobileReader.getFrame().model().lengthPx() / 16;
            float scale = 1;
            scale /= wheelDist * 0.77f;
    ...
    public static void initEntities() {
        ...
        libs.entityRenderer(AutomobilityEntities.BANANA.require(), BananaEntityRenderer::new);
        libs.entityRenderer(AutomobilityEntities.BOX.require(), BoxEntityRenderer::new);
        libs.entityRenderer(AutomobilityEntities.SHELL.require(), ShellEntityRenderer::new);
        libs.entityRenderer(AutomobilityEntities.MUSHROOM.require(), MushroomEntityRenderer::new);
        libs.entityRenderer(AutomobilityEntities.BOMB.require(), BombEntityRenderer::new);
        libs.entityRenderer(AutomobilityEntities.LIGHTNING.require(), LightningEntityRenderer::new);

        libs.modelLayer(BananaModel.MODEL_LAYER);
