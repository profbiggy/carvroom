...
public enum CommonPackets {;
    ...
    public static void sendSpinoutAutomobileDataPacket(AutomobileEntity entity, ServerPlayer player) {
        var buf = new FriendlyByteBuf(Unpooled.buffer());
        buf.writeInt(entity.getId());
        Platform.get().serverSendPacket(player, Automobility.rl("spinout_automobile"), buf);
    }

    public static void sendStopAutomobileDataPacket(AutomobileEntity entity, ServerPlayer player) {
        var buf = new FriendlyByteBuf(Unpooled.buffer());
        buf.writeInt(entity.getId());
        Platform.get().serverSendPacket(player, Automobility.rl("stop_automobile"), buf);
    }

    public static void sendFinishedAutomobileDataPacket(AutomobileEntity entity, ServerPlayer player) {
        var buf = new FriendlyByteBuf(Unpooled.buffer());
        buf.writeInt(entity.getId());
        Platform.get().serverSendPacket(player, Automobility.rl("finished_automobile"), buf);
    }
