...
public class AutomobileEntity extends Entity implements RenderableAutomobile, EntityWithInventory {
    ...
    public void doCountdown(){
        if (stop == 60) {
            ...
            Minecraft.getInstance().gui.setTitle(Component.literal("3").withStyle(style -> style.withColor(0xFFF12D)));
        } else if (stop == 40) {
            Minecraft.getInstance().gui.setTimes(5, 20, 5);
            Minecraft.getInstance().gui.setTitle(Component.literal("2").withStyle(style -> style.withColor(0xFFF12D)));
        } else if (stop == 20) {
            Minecraft.getInstance().gui.setTimes(5, 20, 5);
            Minecraft.getInstance().gui.setTitle(Component.literal("1").withStyle(style -> style.withColor(0xFFF12D)));
        } else if (stop == 1) {
            Minecraft.getInstance().gui.setTimes(5, 20, 5);
            Minecraft.getInstance().gui.setTitle(Component.literal("Go!").withStyle(style -> style.withColor(0xFFF12D)));
        }
    }

    public void doFinishedScreen(){
        int place = finishedPlayers + 1;
        Minecraft.getInstance().gui.setTimes(5, 20, 5);

        if(place == 1){
            Minecraft.getInstance().gui.setTitle(Component.literal("Finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal( place + "st").withStyle(style -> style.withColor(0xFFF12D))));
        }
        else if(place == 2){
            Minecraft.getInstance().gui.setTitle(Component.literal("Finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal( place + "nd").withStyle(style -> style.withColor(0xaed3d4))));
        }
        else if(place == 3){
            Minecraft.getInstance().gui.setTitle(Component.literal("Finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal( place + "rd").withStyle(style -> style.withColor(0xc49254))));
        }
        else{
            Minecraft.getInstance().gui.setTitle(Component.literal("Finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal( place + "th").withStyle(style -> style.withColor(0xbdbdbd))));
        }
    }
    ...
    public void tick() {
        ...
        if(stop > 0){
            ...
            if(level().isClientSide) {
                ...
                if(getControllingPassenger() != null && user.equals(getControllingPassenger().getName().getString())) {
                    doCountdown();
                }
            }
        ...
        if(start){
            ...
            var newBlockBelow = getFeetBlockState().getBlock();

            if((newBlockBelow instanceof BasePressurePlateBlock) && !(oldBlockBelow instanceof BasePressurePlateBlock)){

                lapCount++;
            }

            oldBlockBelow = newBlockBelow;
        }
        ...
        if(lapCount == 4){
            if(!done){
                if(!level().isClientSide) {
                    forNearbyPlayers(200000, false, player -> CommonPackets.sendFinishedAutomobileDataPacket(this, player));
                    ...
                    for(ServerPlayer serverPlayer : getServer().getPlayerList().getPlayers()) {
                        ...
                        if(getControllingPassenger() != null) {
                            if (place == 1) {
                                serverPlayer.sendSystemMessage(Component.literal(getControllingPassenger().getName().getString() + " finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal(place + "st").withStyle(style -> style.withColor(0xFFF12D))));
                            } else if (place == 2) {
                                serverPlayer.sendSystemMessage(Component.literal(getControllingPassenger().getName().getString() + " finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal(place + "nd").withStyle(style -> style.withColor(0xaed3d4))));
                            } else if (place == 3) {
                                serverPlayer.sendSystemMessage(Component.literal(getControllingPassenger().getName().getString() + " finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal(place + "rd").withStyle(style -> style.withColor(0xc49254))));
                            } else {
                                serverPlayer.sendSystemMessage(Component.literal(getControllingPassenger().getName().getString() + " finished ").withStyle(style -> style.withColor(0xFFFFFF)).append(Component.literal(place + "th").withStyle(style -> style.withColor(0xbdbdbd))));
                            }
                        }
                    }
                }
    ...
    public void movementTick() {
        ...
        // Bounce on gel
        if (level().getBlockState(this.blockPosition()).getBlock() instanceof LaunchGelBlock) {
            this.setSpeed(Math.max(this.getHSpeed(), 0.1f), Math.max(this.getVSpeed(), 1.6f)); //normally .9, bowser 1.4, snow 1.6
            this.jumpCooldown = 5;
        ...
        if (braking) {
            this.engineSpeed = Math.max(this.engineSpeed - 0.01f, -0.25f);
        }
        ...
        // if(hit){
            //while engineSpeed != 0{
                //this.engineSpeed = AUtils.zero(this.engineSpeed, 0.15f);
            //}
            //while loop or timer{
                //this.setYRot(getYRot() + 1);
            //}
        ...
        var block1 = new BlockPos((int) getX(), (int) (getY() - 0.05), (int) getZ());
        var block = level().getBlockState(block1).getBlock();

        if (boostSpeed < 0.4f && (block == Blocks.DIRT || block == Blocks.COARSE_DIRT)){
            int layers = 2;
            float cap = stats.getComfortableSpeed() * (1 - ((float)layers / 3.5f));
            engineSpeed = Math.min(cap, engineSpeed);
            this.debrisColor = AUtils.colorFromInt(0x594227);
            this.offRoad = true;
        }
        else if (boostSpeed < 0.4f && block == Blocks.GRASS_BLOCK){
            int layers = 2;
            float cap = stats.getComfortableSpeed() * (1 - ((float)layers / 3.5f));
            engineSpeed = Math.min(cap, engineSpeed);
            this.debrisColor = AUtils.colorFromInt(0x406918);
            this.offRoad = true;
        }
//        else if (boostSpeed < 0.4f && block == Blocks.SAND && !isUnderWater()){
//            int layers = 2;
//            float cap = stats.getComfortableSpeed() * (1 - ((float)layers / 3.5f));
//            engineSpeed = Math.min(cap, engineSpeed);
//            this.debrisColor = AUtils.colorFromInt(0xC2B185);
//            this.offRoad = true;
//        }
        else if (boostSpeed < 0.4f && (block == Blocks.SNOW || block == Blocks.SNOW_BLOCK)){
            int layers = 2;
            float cap = stats.getComfortableSpeed() * (1 - ((float)layers / 3.5f));
            engineSpeed = Math.min(cap, engineSpeed);
            this.debrisColor = AUtils.colorFromInt(0xD0E7ED);
            this.offRoad = true;
...
//        }
    ...
    public void spinOut() {
        spinningOut = 20;
        forNearbyPlayers(200, false, player -> CommonPackets.sendSpinoutAutomobileDataPacket(this, player));
    }
    ...
    public void stop(){
        ...
        lapCount = 0;
        finishedPlayers = 0;
        forNearbyPlayers(200000, false, player -> CommonPackets.sendStopAutomobileDataPacket(this, player));
    ...
    private float calculateAcceleration(float speed, AutomobileStats stats) {
        ...
        return (1 / ((150 * speed) + (18.5f - (stats.getAcceleration() * 5.3f)))) * (0.9f * ((stats.getAcceleration() + 1) / 2));
    }
    ...
    private void consumeTurboCharge() {
        if (turboCharge > LARGE_TURBO_TIME) {
            boost(0.3f, 21); //other tracks
            //boost(0.38f, 38); //track 1
    ...
    private void driftingTick() {
        ...
        if (holdingDrift) {
            if (steering != 0 && !drifting && hSpeed > 0.4f) {
                setDrifting(true);
