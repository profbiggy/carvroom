...
public enum ClientPackets {;
    ...
    public static void initClient() {
        ...
        Platform.get().clientReceivePacket(Automobility.rl("spinout_automobile"), (client, buf) -> {
            int entityId = buf.readInt();
            client.execute(() -> {
                if (Objects.requireNonNull(client.level).getEntity(entityId) instanceof AutomobileEntity automobile) {
                    automobile.spinOut();
                }
            });
        });
        Platform.get().clientReceivePacket(Automobility.rl("stop_automobile"), (client, buf) -> {
            int entityId = buf.readInt();
            client.execute(() -> {
                if (Objects.requireNonNull(client.level).getEntity(entityId) instanceof AutomobileEntity automobile) {
                    automobile.stop();
        ...
        Platform.get().clientReceivePacket(Automobility.rl("finished_automobile"), (client, buf) -> client.execute(() -> AutomobileEntity.finishedPlayers++));
